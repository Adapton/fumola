...
/// The same graph, built inside a region.
///
/// Every adapton operation here is handed back to the machine, so this is the
/// direct check that the delegation boundary disturbs nothing: the same twelve
/// events, the same edge ids, the same meta-times. It reads the graph through
/// Fumola itself, so it needs no Rust plumbing to stay true.
#[test]
fn test_events_do_big() {
    assert_(
        r#"
    do big { force (1 := thunk { force (2 := thunk { 44 }) }) };
    (switch (@(`adapton(`state))) { case (#Graphical(g)) g }).history.events"#,
        r#"
[
  {event = #AddNode(#Symbol(#Nat(1)), #Now, [1]); meta_time = [1]},
